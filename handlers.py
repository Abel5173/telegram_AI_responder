...
def handle_afk_command(message, bot, OWNER_AFK_ref, owner_id_int):
    if message.from_user.id == owner_id_int:
        ...
        bot.reply_to(message, "AFK mode enabled. I'll cover your DMs after 15 seconds if you don't reply.")
...
def handle_back_command(message, bot, OWNER_AFK_ref, owner_id_int):
    if message.from_user.id == owner_id_int:
        ...
        bot.reply_to(message, "Welcome back! I won't auto-respond anymore.")
...
def handle_track_owner_reply(message, last_owner_reply, logger, owner_id_int):
    ...
    logger.info(f"Tracked owner reply in chat {message.chat.id} at {last_owner_reply[message.chat.id]}")
...
def handle_main_message(message, bot, last_owner_reply, owner_id_int, OWNER_AFK_ref):
    ...
    logger.info(f"handle_message triggered by user {user_id} in chat {chat_id}")
    ...
    if OWNER_AFK_ref[0]:
        ...
        logger.info(f"OWNER_AFK is True. Waiting 15 seconds for owner reply before responding to user {user_id}.")
        def delayed_response():
            try:
                time.sleep(15)
                if not owner_replied_recently(chat_id, received_time, last_owner_reply):
                    logger.info(f"No owner reply detected in chat {chat_id} after 15 seconds. Sending intro and AI response.")
                    try:
                        bot.send_message(chat_id, "Abel is not here for the moment, but till he is online you can talk to me, like ask me anything....")
                        logger.info(f"Sent intro message to user {user_id} in chat {chat_id}")
                    except requests.exceptions.RequestException as e:
                        logger.error(f"Network error while sending intro message: {e}")
                        return
                    store_message(user_id, chat_id, 'user', user_message)
                    logger.info(f"Waiting for {RESPONSE_DELAY_SECONDS} seconds before AI response to user {user_id}.")
                    time.sleep(RESPONSE_DELAY_SECONDS)
                    try:
                        bot.send_chat_action(chat_id, 'typing')
                        response_text = generate_response(user_id, chat_id, user_message)
                        bot.send_message(chat_id, response_text)
                        logger.info(f"Sent AI response to user {user_id}: '{response_text}'")
                        store_message(user_id, chat_id, 'bot', response_text)
                    except requests.exceptions.RequestException as e:
                        logger.error(f"Network error while sending AI response: {e}")
                        return
                else:
                    logger.info(f"Owner replied in chat {chat_id} within 15 seconds. No bot response sent.")
            except Exception as e:
                logger.error(f"Unexpected error in delayed_response thread: {e}", exc_info=True)
        threading.Thread(target=delayed_response).start()
    else:
        logger.info(f"OWNER_AFK is False. Bot will not auto-respond to user {user_id}.")
...
def register_handlers(bot, last_owner_reply, owner_id_int, OWNER_AFK_ref):
    ...
    def track_owner_reply(message):
        handle_track_owner_reply(message, last_owner_reply, logger, owner_id_int)

    @bot.message_handler(chat_types=['private'], func=lambda message: True)
    def handle_message(message):
        handle_main_message(message, bot, last_owner_reply, owner_id_int, OWNER_AFK_ref) 