...
class TestHandlers(unittest.TestCase):
    ...
    def test_afk_command_owner(self):
        ...
        self.bot.reply_to.assert_called_with(message, "AFK mode enabled. I'll cover your DMs after 15 seconds if you don't reply.")
    ...
    def test_afk_command_non_owner(self):
        ...
        handle_afk_command(message, self.bot, self.OWNER_AFK, self.owner_id_int)
        self.assertTrue(self.OWNER_AFK[0])
        self.bot.reply_to.assert_called_with(message, "Only the owner can use this command.")
    ...
    def test_back_command_owner(self):
        ...
        self.bot.reply_to.assert_called_with(message, "Welcome back! I won't auto-respond anymore.")

    def test_status_command(self):
        message = MagicMock()
        self.OWNER_AFK[0] = True
        handle_status_command(message, self.bot, self.OWNER_AFK)
        self.bot.reply_to.assert_called_with(message, "Owner is currently AFK.")
        self.OWNER_AFK[0] = False
        handle_status_command(message, self.bot, self.OWNER_AFK)
        self.bot.reply_to.assert_called_with(message, "Owner is currently online.")
