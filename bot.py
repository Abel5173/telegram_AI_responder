...
if OWNER_USER_ID and OWNER_USER_ID.isdigit():
    ...
    logger.warning(
        "OWNER_USER_ID is not set or invalid. " "Bot will respond to all users."
    )
...
def generate_response(
    ...
    logger.info(
        f"Sending context to Ollama for user {user_id}: '" f"{context}'")
    ...
    try:
        while True:
            try:
                ...
                logger.critical(
                    f"Unexpected error in polling: {e}", exc_info=True)
                traceback.print_exc()
                print(
                    "\n[CRITICAL] The bot failed to start or crashed. "
